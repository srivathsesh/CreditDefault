...
creditdata %<>%
  mutate(PerBALinc = ifelse(!is.infinite((BILL_AMT6 - BILL_AMT1)/BILL_AMT6),(BILL_AMT6 - BILL_AMT1)/BILL_AMT6,(BILL_AMT6 - BILL_AMT1)/0.01),
         PerUTILinc = ifelse(!is.infinite((UTIL_6 - UTIL_1)/UTIL_6),(UTIL_6 - UTIL_1)/UTIL_6,(UTIL_6 - UTIL_1)/0.0001)
         )
...
The use of very small values like a penny when denominator dollar amounts are 0 prevented the feature from going to an infinite value.Table 6 Shows the summary of the engineered features. Every feature is as expected.

```{r}

EngFeatNames <- creditdata %>% select(matches("RATIO|Avg|UTIL|MR|6mo|Flag|Max|Age_|ppk|inc")) %>% colnames()
