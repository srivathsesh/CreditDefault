...
```{r}
lattice::barchart(prop.table(table(train$EDUCATION,train$DEFAULT)))
```
...
perf.logit <- getPerformance(trainProc[,c(-29)],validProc[,c(-29)],y_trainProc,validY,logitmdl2$finalModel)
...
plot(rf)
plot(varImp(rf))
perf.rf <- getPerformance(trainProc[,c(-29)],validProc[,c(-29)],y_trainProc,validY,rf$finalModel)
...
glmLogit.perf <- getPerformance(trainProc[,c(-29)],validProc[,c(-29)],y_trainProc,validY,glmLogit$finalModel)
...
getPerformance(trainProc2[,c(-29)],validProc2[,c(-29)],y_trainProc,validY,rf$finalModel)
...
```{r}
getPerformance(trainProcMat[,c(-29)],validProcMat[,c(-29)],y_trainProc,validY,xgb_model$finalModel)
```
...
```{r}
registerDoSEQ()
caret::train(x = trainProc[,c(-29)] %>% data.matrix(), 
                  y = y_trainProc,
                  method = "glmnet",
                  family = "binomial",
                  metric = "ROC",
...
glmLogit.perf <- getPerformance(trainProc[,c(-29)] %>% data.matrix(),validProc[,c(-29)] %>% data.matrix(),y_trainProc,validY,glmnetLogit)
...
rf2Preds <- data.frame(Preds = rownames(varImp(rf2)$importance),Imp = varImp(rf2)$importance) %>% top_n(12)
varImp(glmLogit$finalModel)->ImpglmLogit
glmPreds <- data.frame(Preds = rownames(ImpglmLogit), Importance = ImpglmLogit$Overall)
...
rfPreds <- data.frame(Preds = rownames(varImp(rf)$importance),Imp = varImp(rf)$importance) %>% top_n(12)

# manual extraction of the preds rather than intersection for now
Preds <-union(union(union( union(LassoPreds$Pred[-1],xgbPreds$Preds),rf2Preds$Preds),rfPreds$Preds),glmPreds$Preds[1:8])
...
getPerformance(df[,c(-29)],df_valid[,c(-29)],y_trainProc,validY,nbSelectedPred)
```
