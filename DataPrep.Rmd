...
```{r,fig.cap="Effect of revolving credit on default"}
train %>% 
...
```{r,fig.cap="Effect of Delinqucies"}
train %>% 
...
```{r,fig.cap="Utilization of credit limit's effect on Default"}
...
Payments as a ratio of bill amounts, average payment amounts, continuous decrease in payments/pay ratios over time did not yield statistiscally significant results. The results are not produced in this report for brevity.
...
```{r, eval = T,fig.cap = "left:Tuning results of Cost Complexity; right: variable importance plot"}
...
```{r, fig.cap="Decision tree based on cost parameter tuning"}
partykit::as.party(rpartFit$finalModel) -> prty
library(partykit)
plot(prty,gp = gpar(fontsize = 6),
...
plot(varImp(rpartFit_1))

plot(varImp(rpartFit2_1))
stopCluster(cl)
...
```{r}
partykit::as.party(rpartFit2_1$finalModel) -> prty
library(partykit)
plot(prty,gp = gpar(fontsize = 6),
     inner_panel = node_inner,
     ip_args = list(
