...
train2 %<>% 
  ...
  mutate_at(vars(matches("AGE_|Flag")),as.factor)
...
validate <- creditdata %>% filter(validate == 1) %>% select(!!SelectedFeat2,DEFAULT)
validate %<>% 
  mutate_at(PmtRatioBucketsCol,as.factor) %>% 
  mutate_at(vars(matches("AGE_|Flag")),as.factor)

validProc <- predict(preProc,newdata = validate)
validY <- validProc$DEFAULT
levels(validY) <- c("no","yes")
logit2.roc.val <- pROC::roc(response = validY,predict(logitmdl2$finalModel,newdata = validProc[,c(-3,-30)]))
plot(logit2.roc,legacy.axes = T, asp = NA, col = "blue")
plot(logit2.roc.val,legacy.axes = T, asp = NA, col = "red",add = T)

pROC::auc(logit2.roc)
pROC::auc(logit2.roc.val)

```


```{r}
getPerformance <- function(trainX,validX,trainY,validY,model, positive = "yes", rowNames = c("train","valid"),cutoff = 0.5, title = ""){
