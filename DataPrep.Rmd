...
```{r GenderEduMS,fig.cap = "GENDER,EDUCATION & MARITAL STATUS' effect on DEFAULT",fig.pos='asis',fig.height=4,fig.show='asis'}
...
```{r rpart1, eval = T,fig.cap = "left:Tuning results of Cost Complexity; right: variable importance plot",fig.height=4,fig.pos='asis',fig.height = 4}
...
##### 5.8.1.1.1 Performance of Decision tree with complexity parameter tuning

`r figr('model1', TRUE, type="Figure")` shows the performance metrics of the decision tree model in classifying the defaulters. The model has F1 score of 0.45 and 0.39 for training and validation data sets respectively. The sensitivity of the model is 0.33 and 0.29 for training and validation set respectively. There is opportunity for improvement in capturing more of the defaulters.
...
```{r varImpDT,eval = T,fig.cap="Decision tree based on depth of tree",fig.pos = "asis",message=F, warning=F,fig.height=4}
...
```{r depthTree,eval = T, fig.cap = "Decision tree pruned by depth of tree", fig.pos="asis", message=F, warning = F,fig.height=4}
...
```
`r figr('model2', TRUE, type="Figure")` shows the performance of depth of tree tuned decision tree model. It's performance is identical to complexity tuned model. However the F1 score on the validation set is slightly better in this model. 

```{r model2,fig.cap = "model1 decision tree cost depth of tree tuning"}
...
# getPerformance(xOneR,validOneR,xOneR$DEFAULT,validOneR$DEFAULT,baselinemod,title = "OneR Decision tree single predictor") -> baselinemodstats
```

```{r oneRPerf,fig.cap = "OneR model performance"}
...
```{r oner2,fig.cap = "oneR re-run decison tree summary",fig.height=4}
#------------------------------------------------------------
...
A filtering based method is used to determine redundant features to be removed for modeling purposes. The features $Max\_Bill\_Amt$, $DLQ\_Max$ ,$utilFlag$ and $MARRIAGE.2$ (marital status - single) were not adding new information to the predictor space due to their correlation with other variables. Hence they were removes from the predictor space. `r figr('correlations', TRUE, type="Figure")` shows correlations amongst the features. It must be noted that some categorical variables were converted to numeric to observe correlations. The results of such an analysis must be carefully interpreted. In this case categirical variables with 2 levels are compared with categorical variables with 2 levels (1 and 0) for determination of redundant information in the predictor space.

```{r correlations,fig.cap="Correlations amongst predictors",fig.pos="asis",fig.height=4}
#--------------------------------------------------------------
...
The performance table below shows an improved AUC than the single predictor decision trees, however the F1 statistic is poorer. `r figr('mdl4', TRUE, type="Figure")` shows the performance of the simple logistic regression model while the AUC shows an improvement of 77%, the model lacks sensitivity and has a poorer F1 score of 0.26 each for training and validation data sets.
...
```{r mdl4,fig.cap = "model1 decision tree cost complexity tuning",fig.align='center',fig.height=4,fig.show='asis'}
...
```{r stpwise,warning=F, message = F, fig.cap= "feature imporance based on stepwise selection ", fig.cap = 4}
#glmLogit.perf$Perf
...
The model though has a AUC of 77%, the sensitivity is 16% with F1 as 0.25 and 0.27 for train and validation data sets respectively. Different threshold probability cut off to classify as defaulting customer was explored, however 0.5 threshold had the maximum F1. The sensitivity of the model is poor at 0.16. 

```{r, fig.cap = "Stepwise logit regression performance"}
...
```{r,fig.cap = "variable importance",fig.height = 4, fig.pos="asis"}
# registerDoSEQ()
...
`r figr('lassoPerf', TRUE, type="Figure")` shows Lasso regression has made the best classifications so far. With Sensitivity at 33 % and 34% for train and validation data sets respectively. With a F1 score of 0.43 for both data sets. 

```{r lassoPerf,fig.cap = "Lasso regression performance", fig.pos = "asis",}
...
Random forest methods were tuned with different predictors for each iteration. `r figr('rfimp', TRUE, type="Figure")` shows the tuning results and the top 20 important features. The threshold (probability cut off) to predict the positive class (defaulters) were tuned for the training data by optimizing it for the F1 score. While a threshold of 0.4 yielded the best result in the training data, it did not yield a significant improvement in the validation data. Hence the threshold was retained at its defualt of 0.5. `r figr('rfperf', TRUE, type="Figure")` shows the model overfit the training data with AUC as 1 and sensitivity of 0.96 and performed poorly on the validation data set relative to Lasso regression model.
...
```{r rfperf,fig.cap = "Random forest performance"}
...
An extreme gradient boosting model was tuned with a constant learning rate of 10% and L2 regularization of 0. The model was tuned over the depth of trees and 10% to 90% of the predictor space. The tuning was performed over 100% and 75% of the training samples. `r figr('xgbvarimp', TRUE, type="Figure")` below shows the model tuning results and the variable importance. 
...
```{r xgbvarimp, fig.cap = "xgBoost tuning"}
plot(xgb_model, main = "xgboost tuning") -> xgb1
plot(varImp(xgb_model),top = 12, main = "Top 20 predictors - xgboost") -> xgbvarimp
gridExtra::grid.arrange(xgb1,xgbvarimp, ncol = 2)
