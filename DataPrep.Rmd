...
Table 23 lists the consolidated performance. $F1*$ is defined as $F1* = 2*\frac{precision * recall}{(precision + recall)}$  and $\% VAR\ recovered = \frac{Predicted\ VAR}{Actual\ VAR}$. $VAR$ is value at risk. $F1*$ score is given more importance as it takes into account the true positive rate and false positive rate; followed by sensitivity as there is interest in correctly identifying the defaulters in the customer pool. The next order of importance is given to $\% VAR\ recovered$ as there is interest in budgeting as accurately as possible for value at risk write off. 

When the table is arranged in descending order of $F1*$ , $Sensitivity$ and $\% VAR\ recovered$. Naive Bayes model jumps out as the best performing model.
