...
```{r rpart1, eval = T,fig.cap = "left:Tuning results of Cost Complexity; right: variable importance plot",fig.pos='asis',fig.height = 3}
...
`r figr('depthTree', TRUE, type="Figure")` shows the depth of tree tuned decision tree and  `r figr('model2', TRUE, type="Figure")` shows its performance. It's performance is identical to complexity tuned model. However the F1 score on the validation set is slightly better in this model. 
...
```{r, fig.cap = "model 5 - Stepwise logit regression performance"}
...
```{r,fig.cap = "variable importance",fig.height = 4}
# registerDoSEQ()
...
An extreme gradient boosting model was tuned with a constant learning rate of 10% and L2 regularization of 0. The model was tuned over the depth of trees and 10% to 90% of the predictor space. The tuning was performed over 100% and 75% of the training samples. `r figr('xgbtuning', TRUE, type="Figure")` and  `r figr('xgbVarImp', TRUE, type="Figure")` shows the model tuning results and the variable importance of top 20 important variables. 
...
```{r xgbtuning, fig.cap = "xgBoost tuning", fig.height=3,fig.width=4}
plot(xgb_model,ylab=list(cex=1), xlab = list(cex = 1),main = list(label =  "xgboost tuning",cex = 1))
```


```{r xgbVarImp, fig.cap = "xgBoost variable importance",fig.height=4}
...
The union of top 10 predictors from each of the models fit in section 6 are used for further modeling the classification model. The chosen preictors are as shown in table 21.
