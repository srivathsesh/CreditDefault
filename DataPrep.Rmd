...
```{r}
lattice::barchart(prop.table(table(train$SEX,train$DEFAULT)))
...
```{r}
lattice::barchart(prop.table(table(train$EDUCATION,train$DEFAULT)),grid=T)
...
```{r}
lattice::barchart(prop.table(table(train$MARRIAGE,train$DEFAULT)))
library(ggmosaic)
ggplot(data = train) + geom_mosaic( aes(x=product(EDUCATION,SEX),fill = DEFAULT)) + facet_wrap(~MARRIAGE)
...
corrplot::corrplot(cbind.data.frame(train2,EDUDUM,MARDUM) %>% select(-EDUCATION,-MARRIAGE,-DEFAULT) %>% mutate_if(is.factor,as.numeric) %>% cor(.))
...
perf.logit <- getPerformance(trainProc[,-which(names(trainProc) %in% 'DEFAULT')],validProc[,-which(names(trainProc) %in% 'DEFAULT')],y_trainProc,validY,logitmdl2$finalModel)
...
plot(rf)
plot(varImp(rf))
perf.rf <- getPerformance(trainProc[,-which(names(trainProc) %in% 'DEFAULT')],validProc[,-which(names(validProc) %in% 'DEFAULT')],y_trainProc,validY,rf$finalModel)
...
glmLogit.perf <- getPerformance(trainProc[,-which(names(trainProc) %in% 'DEFAULT')],validProc[,-which(names(validProc) %in% 'DEFAULT')],y_trainProc,validY,glmLogit$finalModel)
...
getPerformance(trainProc2[,-which(names(trainProc2) %in% 'DEFAULT')],validProc2[,-which(names(validProc2) %in% 'DEFAULT')],y_trainProc,validY,rf$finalModel)
...
```{r}
getPerformance(trainProcMat[,-which(colnames(trainProcMat) %in% 'DEFAULT')],validProcMat[,-which(colnames(validProcMat) %in% 'DEFAULT')],y_trainProc,validY,xgb_model$finalModel)
```
...
glmnetLogit.perf <- getPerformance(trainProc[,-which(names(trainProc) %in% 'DEFAULT')] %>% data.matrix(),validProc[,-which(names(validProc) %in% 'DEFAULT')] %>% data.matrix(),y_trainProc,validY,glmnetLogit)

glmnetLogit.perf$Perf
```
...
getPerformance(trainProc[,which(names(trainProc) %in% Preds)],validProc[,which(names(validProc) %in% Preds)],y_trainProc,validY,logitSelectedPred$finalModel)
```
...
getPerformance(df[,PredsLogit],df_valid[,PredsLogit],y_trainProc,validY,logitSelectedPred2$finalModel)
...
getPerformance(df[,-which(names(df) %in% 'DEFAULT')],df_valid[,-which(names(df_valid) %in% 'DEFAULT')],y_trainProc,validY,nbSelectedPred)
```
...
model <- keras_model_sequential() %>% 
    # first hidden layer
  layer_dense(units = 16,activation = "relu",input_shape = length(Preds)) %>% 
    # add drop out layer
