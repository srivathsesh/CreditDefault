...
```{r}
LassoPreds <- Importance %>% top_n(10,abs(Coef))
...
Preds <-intersect(intersect(intersect( intersect(LassoPreds$Pred[-1],xgbPreds$Preds),rf2Preds$Preds),rfPreds$Preds),glmPreds$Preds[1:8])
...
getPerformance(df[,c(-29)],df_valid[,c(-29)],y_trainProc,validY,logitSelectedPred$finalModel)
