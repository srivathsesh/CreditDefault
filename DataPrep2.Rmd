---
title: |
  | \vspace{8cm} \LARGE{Credit Default Risk Modeling}
  | MSDS 498 Capstone
...
Current Expected Credit Losses (CECL) is an accounting standard that was released by Financial Accounting Standards Board (FASB) in June 2016. CECL mandates banks to report on expected losses over the life of the loan rather than just the losses incurred at the time of their financial reporting. Banks are required to report on the allowance for credit losses as accurately as possible, which calls for accurate predicitve models for identifying customers who are to default. And estimate the losses from their accounts. 

@yeh_comparisons_2009 in their paper explore six different modeling methods - discriminant analysis, logistic regression, Bayes classifier, nearest neighbor, artificial neural networks and classification trees and their applications on real cardholders’ credit risk data in Taiwan (“UCI Machine Learning Repository: Default of credit card clients Data Set,” n.d.). They quantify the performance of the models, but show very little detail on what variables were used in their model and if they used derived variables for modeling.

This paper attempts to enhance Yeh and Lien's work by exploring derived variables from the data (such as, credit utilization, payments to bill ratio, prior delinquencies in months and number of bills delinquent etc.) and their effect on the model performance. Yeh and Lien show that artifical neural network was their best performing model, though they do not discuss on the architecture of the network. In this paper, Naive Bayes model outperformed the neural network model with a precision of 0.47, recall of 0.5 and a F1 score (defined as $\frac{2*(precision * recall)}{precision + recall}$, see **appendix** for more details on why this definition is conservative) of 0.48. This paper also attempts to quantify the model's ability to correctly capture the Value at Risk (VAR). This is done by introducing a new metric % VAR recovered, which is a ratio of loss as identified by the model to the actual loss.

In section 2, the summary of the raw data is presented and in section 3, new features derived from the data are discussed. The predicitve ability of the derived features and others are explored in section 4 and in section 5 model based feature selection is discussed. The models are discussed in section 7 and the results are summarised in section 8. 
...
Ratio of payments to bill amounts, average payment amounts, continuous decrease in payments/pay ratios over time did not yield statistically significant results when explored graphically using density plots. However, in the section 4.8.1.1, decision trees help bring out signal from payment variables and the effect of payments is discussed in that section below.

## 4.6 Effect of Age

Figure 1 in section 3 shows relatively higher chances of defaulting in the early ages (21-24) and in the age group past mid-life 45 years and above. The increased chances in default past mid-life may be potentially attributed to loss in employment and or health related events in life.
...
The best tuning for cost parameter based on 10 fold cross validation was 0.015. `r figr('rpart1', TRUE, type="Figure")` shows the tuning and the importance of the various predictors. It is to be noted that maximum delinquencies (DLQ_Max) was removed from the predictors list for the model shown below. This was done to remove any effect of correlation with DLQ_Total. 
...
The model shows an improved AUC than the single predictor decision trees, however the F1 statistic is poorer. `r figr('mdl4', TRUE, type="Figure")` shows the performance of the simple logistic regression model. Though the model has an improved AUC of 77%, it lacks sensitivity and has a poorer F1 score of 0.25,0.26 and 0.25 for training, validation and test data sets. The % VAR recovered of 100%, 102% and 115% for train,validation and test data sets are better than the previous models.
...
**All models are tuned based on 10 fold cross validated samples on the training data set and tested on the validation and test data**
...
The model though has a AUC of 77%, the sensitivity is 16% with F1 as 0.25 and 0.27 for train and validation data sets respectively. The results were similar for the test data. Different threshold probability cut off values to classify defaulting customers were explored, however 0.5 threshold had the maximum F1. The sensitivity of the model is poor at 0.16. `r figr('stpwiseperf', TRUE, type="Figure")` shows the performance statistics.
...
`r figr('lassoPerf', TRUE, type="Figure")` shows Lasso regression has made the best classifications so far. With Sensitivity at 33 % and 34% for train and validation data sets respectively. With a F1 score of 0.43 for both data sets. The % VAR recovered is similar to that of stepwise logit model. The performance on the test data is identical to that of train and validation sets. 
...
The union of top 10 predictors from each of the models fit in section 5 are used for further modeling the classification model. The chosen predictors are as shown in table 21.
...
A prediction model that uses majority class votes by all models fit in the sections above was attempted. The results were not very impressive. The results were poorer with a sensitivity of 0.33 and F1 score of 0.43 across training, validation and test data sets. The % VAR recovered was 0.63 for the test data, which is much lower than the other models. The model performance is shown in `r figr('ensemble', TRUE, type="Figure")`

```{r ensemble,fig.cap = "model 13 - Majority vote ensemble model performance"}
knitr::include_graphics('modelResults/model13.png',auto_pdf = T)
```

# 8. Comparison of Results

In this section, the choice of metrics to be used to compare between models and select the best to be used for the business problem.

## 8.1 Choice of metrics 

Banks would ideally want to identify customers who are likely to default to minimize their exposure. Potential actions could be to reduce the credit limit or not offer increased credit, impose transaction limits per day or month. Therefore a model that correctly identifies a defaulting customer is necessary. False positives could result in customers leaving and false negatives increases the exposure to risk. F1 score is a metric that takes into account false positive and true positive rates in the form of combining precision and recall.

\begin{center}  $F1* = 2*\frac{precision * recall}{(precision + recall)}$ \end {center}

Once can derive another form of F1 score from the above equation, which is as below

\begin{center}   $F1 = \frac{2*True\ Positive\ Rate}{2* True\ Positive\ Rate + False\ Negative \ Rate + False\ Positive\ Rate}$ \end {center}

But this form of F1 score happens to over estimate the F1 score due to precision of digits, especially when $False\ Positive\ Rate$ is high. Hence we use $F1*$ to be conservative. See **Appendix** for more details.

Banks would like to reduce their overall exposure by making sure they identify all the potential defaulters from their customer population. So looking at the $recall$ metric in addition to $F1* score$  would be useful.

Finally, the model should be able to capture customers who are to default on large amount of money. Therefore  percent or ratio of Value At Risk (VAR) identified by model to the actual VAR for the bank (sum of money defaulted by customers) could be a metric that will be useful to consider for model choice. We will call this metric $\% VAR\ recovered$. 

\begin{center}  $\% VAR\ recovered = \frac{Predicted\ VAR}{Actual\ VAR}$ \end {center}

## 8.2 Model choice

Table 23 lists the consolidated performance. When the table is arranged in descending order of $F1*$ , $Sensitivity$ and $\% VAR\ recovered$. Naive Bayes model jumps out as the best performing model.The Naive Bayes model performance is highlighted in red in the table. The Area Under the Curve (AUC) is at the mean of all model's AUC. The performance of the model on test data was considered more important.

```{r comparisontable}
readxl::read_xlsx('modelResults/confusion_matrix_table.xlsx',sheet = "ConsolidatedResult") -> ResultsConsolidated

ResultsConsolidated %>% 
  kable("latex",digits = 2,booktabs = T,caption = "Model performance comparison") %>% 
  kable_styling(latex_options = c("striped","hold_position"),font_size = 7) %>% 
  collapse_rows(columns = 1:2, latex_hline = "major", valign = "middle") %>% 
  row_spec(31,color = "red",bold = T)
```
...
Since the global financial crisis in 2008 there are regulatory requirements for banks to report on their risk exposure. On June 2016 Financial Accounting and Standards Board (FASB) issued a new accounting method called Current Expected Credit Losses (CECL). Per CECL, expected losses over the life of the loan is to be reported and a reserve needs to be set aside by banks to cover for the exposure. This requirement mandates the use of effective predictive models to predict potential losses or defaults by customers during the life of the loan. The predicitive model not only helps CECL reporting needs, but also helps banks minimize the risk by taking actions such as reducing the credit limit on the credit card, imposing transaction limits on daily or monthly basis or loan re-adjustment for the customer etc. 

This paper explored the various modeling techniques that could be used for predicting customers defaulting on their credit card bills. The models identified important predictors such as prior number of delinquencies, growth of balance month over 6 months, average monthly payments, credit utilization etc. (refer table 21 for the complete list). The Naive Bayes model performed the best with a F1 score of 0.48, recall of 0.5 and a precision of 0.47. The performance was not as strong as expected. 

To get better model performance, data issues could be addressed. For example, in the data provided there were over 500 customers whose bill was 0, but were marked defaulting customer. Secondly additional features such as home ownership, credit score could be explored for prediciting defaulting customers. Thirdly, upsampling of defaulting customers and/or using SMOTE alogorithm could be used to reduce the effect of low prevalance of defaulting customers on the model training. Additionally, different modeling approaches such as CHAID, SVM and different neural network architecture including Boltzmann Machines could be tried.
...
Here the over estimation of F1 score when using the below formula is proved.
...
Before proving the over estimation, we'd prove that below formula is the same as the formula above.

\begin{center} $F1* = 2*\frac{precision * recall}{(precision + recall)}$ \end{center}


Below is the depiction of confusion matrix

```{r,fig.align='center'}
knitr::include_graphics('modelResults/confmat.png',auto_pdf = T)
```

\begin{center}  $Precision = \frac{TP}{TP + FP}$ \end{center}


\begin{center}  $Recall = \frac{TP}{TP + FN}$ \end{center}

Therefore,

\begin{center} $F1* = 2*\frac{\frac{TP}{TP + FP} * \frac{TP}{TP + FN}}{(\frac{TP}{TP + FP} + \frac{TP}{TP + FN})}$ \end{center}

\begin{center}  $= \frac{2*TP^{2}}{2*TP^{2} + TP(FN+FP)}$ \end{center}

\begin{center}  $= \frac{2*TP}{2*TP + FN+FP}$ \end{center}


Let us say we have the following confusion matrix

```{r}
data.frame(Actual = c(rep(0,1383),rep(1,900)),
           Predicted = c(rep(0,1332),rep(1,51),rep(0,53),rep(1,847))) -> df
  df %>% table()
```
...
\begin{center} Precision = `r 847/(847+51)` \end{center}

\begin{center} Recall =  `r 847/(847+53)` \end{center}

\begin{center} $F1* = 2*\frac{0.9432071*0.9411111}{(0.9411111+0.9432071)}$ \end{center}

\begin{center} F1* = `r 2*( 0.9432071*0.9411111)/(0.9411111+0.9432071)` \end{center}


When we re-write the table in row-wise proportions, we get the TPR, FPR TNR and FNR
...
\begin{center} $F1 = \frac{2*0.94111111}{2*0.94111111 + 0.05888889 + 0.03687636}$ \end{center}

\begin{center} F1 = `r (2*0.94111111)/(2*0.94111111 + 0.05888889 + 0.03687636)` \end{center}

We don't see much difference between F1* and F1. However we'll now inflate the False Positive Rate (FPR)

Here is how the confusion matrix with inflated false positives look like

```{r}
data.frame(Actual = c(rep(0,1383),rep(1,900)),
           Predicted = c(rep(1,1383),rep(0,53),rep(1,847))) -> df2
  df2 %>% table()
```

\begin{center} Precision = `r 847/(847+1383)` \end{center}

\begin{center} Recall =  `r 847/(847+53)` \end{center}

\begin{center} $F1* = 2*\frac{0.3798206*0.9411111}{(0.9411111+0.3798206)}$ \end{center}

\begin{center} F1* = `r 2*( 0.3798206*0.9411111)/(0.9411111+0.3798206)` \end{center}

now again, we re-write the table in row-wise proportions, we get the TPR, FPR TNR and FNR


```{r}
prop.table(table(df2), margin = 1)
```
...
\begin{center} F1 = `r (2*0.94111111)/(2*0.94111111 + 0.05888889 + 1)` \end{center}

Now we see that there is a difference between the two estimates of the score, which is approximately 15% difference.
